...
c     PARAMETER(NFEL=3000)
     ...
     &     PH(NL),DRDT(NL),RECT(5,NL),PHET(NL)
      ...
      if(iel==11.and.ion==5.and.te<1.e3) then
         n=5
      elseif(iel==14.and.ion==9.and.te<1.5e3) then
         n=3
      elseif(iel==14.and.ion==8.and.te<1.e3) then
         n=2
      elseif(iel==14.and.ion==7.and.te<1.e3) then
         n=8
      elseif(iel==14.and.ion==6.and.te<1.e3) then
         n=9
      endif
      ...
      if(itime.le.3.or.abxa.gt.1.e-8) then
         ...
         ieli=iel
         ioni=ion
         if(iel==13.and.ion==5) then
            do kk=1,21
               tei=1.e2*10**((kk-1)*0.3)               
               call el_coll_rate_chianti(ionnr(ieli,ioni),ieli,ioni,
     &              nmax(ionnr(ieli,ioni)),tei)
            enddo
         endif
         call el_coll_rate_chianti(ionnr(ieli,ioni),ieli,ioni,
...
c     put the data from chianti into the usual arrays
     ...
     &           EXP((E_ion(ionnr(iel,ion))-E(ionnr(iel,ion),J))/TEV)
     &           /Te**1.5)
         enddo  
         ITCON=1
      ...
      implicit none
      integer nfel,ncr,nio,md,mdp1,nl,nlp1
      integer nups,ionnr,ionnri,nmaxi,nchi22
      include "parameters.h"
      real*8 upsil,c,ci,g,e,e_ion,a,wl,dcdt,dcidt,te,tex,omega
...
c for Fe I and II                
               omega=upsil(ionnri,iu,il)
            else
c for all others from Chianti               
               omega=upsil(ionnri,il,iu)
            endif
c same as 8.63e-6 * .....            
     ...
     &     c(nchi22,nl,nl),ci(nchi22,nl),nmax(nchi22),ionnr(14,26)
      ...
      if(iel.eq.3.and.ion.eq.2) then
         ...
         inew=1
         e_ion(ionnr(iel,ion))=24.383143 
c         write(6,*)nmax(ionnr(iel,ion))
...
c     if(iel.eq.5.and.ion==2) then
         ...
         elseif(cfform==1) then
            ...
            do i=1,100000
               ...
               if(iu.le.nmax(ionnr(iel,ion))) then
                  a(ionnr(iel,ion),iu,il)=a21
                  upsil(ionnr(iel,ion),iu,il)=omega
               endif            
               if(iu.le.nmax(ionnr(iel,ion))) then
                  de=e(ionnr(iel,ion),iu)-e(ionnr(iel,ion),il)
                  wll=12398.41/de
     ...
     &     scups_knot,iumaxil
      if(init==1) then
         ...
         elseif(iel==5.and.ion==2) then
            open(11,file='./ATDAT/o_2_rev.upsdata')
